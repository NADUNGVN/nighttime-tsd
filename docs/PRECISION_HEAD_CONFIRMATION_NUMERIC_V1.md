...
The calibration producer is inspected but its full loader is never dispatched. On exactly three accepted anchors, the child exercises the pinned `YOLODataset.load_image` component with `rect_mode=True, resize_short=False`, then validation `LetterBox(new_shape=(640,640), auto=False, scale_fill=False, scaleup=False, center=True, padding_value=114)`, followed by the actual `Format._format_img`-equivalent RGB/CHW uint8 layout and the repository helper's explicit `float32 / 255` stream representation. The `.npy` candidate passed to `load_image` is always a guaranteed-absent path inside a fresh private `TemporaryDirectory`; the original adjacent dataset/materialization `.npy` is never passed, read, or deleted. The report stores these calibration stages separately from inference stages, including preliminary resize, ratio/padding, dtypes, ranges, hashes and cache-isolation evidence. It records `calibration_component_called=true`, `calibration_loader_called=false`, `dataset_instantiated=false`, `labels_read=false` and `image_cache_read=false`; source hashes are `inspected` evidence and are not promoted to producer execution claims. No full 3,072-image materialization, cache or calibration cache is created.
...
## H1–H3 schema and lifecycle repair contract

The accepted readiness metadata is the single source for native head identity. The runner requires `accepted_contract.head.type`, `head.index` and `head.end2end`, with the canonical values `Detect/22/false` for YOLOv8n and `Detect/23/true` for YOLO26n. Missing, malformed or model-mismatched metadata is a schema error with expected and observed values; the runner does not fall back to an empty or invented `expected_contract` record. The same normalized contract is used by native head validation and reference semantics.

Child failure evidence is model-owned: `models/<model>/failure.json` and that model's `partial/` files only. The parent owns the complete publishable inventory, including shared plan/manifest/report and both model logs. Each child records ordered stage history plus attempted/completed `source_cpu_fp32` and `onnx_cpu` counts, including failures before, during or after a forward. This scope metadata describes artifact ownership and lifecycle; it is not evidence that model tensors were mixed.

The local integration boundary uses the accepted readiness/graph-v4 metadata and actual fixture planning, head validation, `_model_plan`, `run_model_child`, output extraction, comparison and report aggregation. Only external runtime/file evidence is injected into faithful CPU doubles; no unconditional child success stub replaces the runner path. The existing v1 output is immutable. A v2 run records a `previous_attempt` link and inventory for `precision_head_confirmation_numeric_v1` before writing the fresh `precision_head_confirmation_numeric_v2` root.

## Authorized bounded server CPU command after H1–H3 implementation

After the H1–H3 tests in the Luna handoff pass, the operator may pull the reviewed commit, verify that the v1 output is present and the v2 output root is absent, and run the following single foreground command in the already provisioned environment:

Pull/check one line: `cd /home/ubuntu/Dung_TDTU/nighttime-tsd-new && env -u LD_LIBRARY_PATH -u LD_PRELOAD PATH=/usr/bin:/bin /usr/bin/git pull --ff-only origin master && env -u LD_LIBRARY_PATH -u LD_PRELOAD PATH=/usr/bin:/bin /usr/bin/git rev-parse HEAD && test -d results/measurement_audit_v1/server_precision_head_confirmation_readiness_v2 && test -d results/measurement_audit_v1/precision_head_confirmation_graph_audit_v4 && test -d results/measurement_audit_v1/precision_head_confirmation_graph_prep_v2 && test -f results/measurement_audit_v1/precision_head_confirmation_graph_prep_v2/models/yolov8n/model.onnx && test -f results/measurement_audit_v1/precision_head_confirmation_graph_prep_v2/models/yolo26n/model.onnx && test -f results/measurement_audit_v1/precision_head_confirmation_numeric_v1/numeric_manifest.json && if [ -e results/measurement_audit_v1/precision_head_confirmation_numeric_v2 ]; then echo OUTPUT_EXISTS; else echo OUTPUT_ABSENT; fi`
