...
Regression suite phải được chạy lại mỗi lần bàn giao; mốc local hiện tại ghi 52 test. Compile/CLI đã kiểm tra; không có TensorRT GPU ở local để xác nhận end-to-end build. Server dùng venv `local/g0_size_env` kế thừa nighttime-tsd; không upgrade packages. Cần khoảng vài GB trống cho staged tensors, ONNX, engines và verbose logs. Script giữ mọi artifact trên server, không tự xóa. Không auto-resume partial output.
...
```bash
conda activate nighttime-tsd && cd /home/ubuntu/Dung_TDTU/nighttime-tsd-new && local/g0_size_env/bin/python scripts/uniform_build_repeat.py --out-dir results/measurement_audit_v1/server_uniform_build_repeat_v1 --confirm-desktop-process 644963=/snap/snapd-desktop-integration/391/usr/bin/snapd-desktop-integration --confirm-desktop-process 644977=/snap/snapd-desktop-integration/391/usr/bin/snapd-desktop-integration
```
...
Preflight hiện phân loại từng CUDA process từ `nvidia-smi`. Ngoại lệ desktop không yêu cầu đọc `/proc/<pid>/exe`, nhưng cần operator xác nhận chủ động đúng dòng hiện tại bằng `--confirm-desktop-process PID=PATH`; runner chỉ chấp nhận khi PID và path khớp chính xác snapshot `nvidia-smi` vừa đọc. Path phải khớp allowlist desktop hẹp gồm Snap desktop integration với revision số, Xorg (`/usr/lib/xorg/Xorg` hoặc `/usr/bin/Xorg`), GNOME Shell và GNOME session binary ở các đường dẫn tuyệt đối đã định trước. Tên process, UID, VRAM thấp hoặc PID lịch sử không cấp quyền ngoại lệ. Python/training, process build/inference, RustDesk, path desktop sai, xác nhận không khớp, path `[Not Found]` và dòng output không parse được đều fail closed.

Mỗi GPU snapshot ghi raw process output, reported path, classification, lý do, phương thức xác minh, danh sách xác nhận operator, telemetry status và `process_guard` disclaimer. Process desktop được allow vẫn xuất hiện trong snapshot; điều đó không chứng minh GPU không có nhiễu hoặc GPU được cô lập tuyệt đối. `ensure_idle()` giữ nguyên GPU phase lock, không kill/pause process và đối chiếu lại PID/path với snapshot hiện tại trước build/capture. Nếu workload cạnh tranh xuất hiện sau preflight, snapshot ghi `external_workload_detected` và báo cáo cuối đánh dấu review-required; telemetry chỉ là các điểm lấy mẫu, không loại trừ workload giữa hai snapshot. Các test offline bao phủ desktop có/không xác nhận, PID/path không khớp, training VRAM thấp, process không phân loại, thiếu telemetry và workload cạnh tranh; không test nào coi đó là TensorRT end-to-end.

Trên shared lab server có thể chạy với process desktop sau khi operator đối chiếu output hiện tại, ví dụ thêm vào lệnh runner: `--confirm-desktop-process 644963=/snap/snapd-desktop-integration/391/usr/bin/snapd-desktop-integration --confirm-desktop-process 644977=/snap/snapd-desktop-integration/391/usr/bin/snapd-desktop-integration`. Không dùng lại PID nếu process đã thay đổi; lấy PID/path mới từ snapshot trước mỗi study. Nếu không truyền xác nhận, desktop process vẫn bị chặn. Đây là xác nhận theo telemetry `nvidia-smi`, không phải xác minh qua `/proc`.
