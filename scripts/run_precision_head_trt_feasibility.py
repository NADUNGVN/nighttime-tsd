...
def validate_ort_contract(contract: dict[str, Any]) -> None:
    ...
    if contract.get("input") != EXPECTED_INPUT:
        raise FeasibilityUnresolved(f"ORT input contract differs: {contract.get('input')!r}")
...
def build_engine(trt: Any, onnx_path: Path, model: str) -> tuple[Any, dict[str, Any]]:
...
def execute_trt_once(engine: Any, runtime: dict[str, Any], input_np: Any, model: str, device: int) -> Any:
    ...
    context.set_tensor_address(EXPECTED_INPUT["name"], int(input_tensor.data_ptr()))
    context.set_tensor_address(EXPECTED_OUTPUT["name"], int(output_tensor.data_ptr()))
...
def runtime_loader(config: dict[str, Any], device: int) -> dict[str, Any]:
    ...
    runtime.update({"trt": trt, "np": np, "torch": torch, "packages": {"expected": expected, "observed": observed}, "device": device})
...
def _child_state(model: str) -> dict[str, Any]:
    return {"model": model, "status": "running", "stage": "not_started", "stage_history": [], "forward_counts": {"trt_application_enqueue": {"attempted": 0, "completed": 0}, "onnx_cpu_reference_call": {"attempted": 0, "completed": 0}, "native_forward": {"attempted": 0, "completed": 0}}, "records_written": 0}
...
def run_model_child(repo: Path, plan: dict[str, Any], model: str, out_dir: Path, state: dict[str, Any] | None = None, runtime_loader: Callable[..., dict[str, Any]] = runtime_loader, snapshot_fn: Callable[..., dict[str, Any]] = snapshot_gpu) -> dict[str, Any]:
    ...
    telemetry_before = snapshot_fn(desktop, background, int(plan["runtime_requirements"]["device_argument"]))
    ...
    runtime = runtime_loader(config, int(plan["runtime_requirements"]["device_argument"]))
    ...
    validate_ort_contract(ort_contract)
    ...
    with tempfile.TemporaryDirectory(prefix=f"{STUDY}_{model}_") as scratch:
        ...
        engine, build_evidence = build_engine(runtime["trt"], onnx_path, model)
        ...
        telemetry_during_build = snapshot_fn(desktop, background, int(plan["runtime_requirements"]["device_argument"]))
        ...
        with records_path.open("x", encoding="utf-8", newline="\n") as records_handle, trace_path.open("x", encoding="utf-8", newline="\n") as trace_handle:
            for index, row in enumerate(fixture_rows):
                ...
                trt_output = execute_trt_once(engine, runtime, input_np, model, int(plan["runtime_requirements"]["device_argument"]))
                ...
                ort_output = _finite_output(runtime["np"], ort_outputs[0], model, "ORT reference")
                reference_private_path = scratch_path / f"onnx_reference_{index:02d}.bin"
                reference_private_path.write_bytes(ort_output.tobytes(order="C"))
                trt_detections = bridge.application_postprocess(model, trt_output, trace, runtime)
                ort_detections = bridge.application_postprocess(model, ort_output, trace, runtime)
                write_jsonl_line(records_handle, {"schema_version": SCHEMA_VERSION, "study": STUDY, "model": model, "order": index, "image": row["image"], "input": {"source_sha256": row["expected_sha256"], "source_bytes": row["expected_bytes"], "shape": list(input_np.shape), "dtype": str(input_np.dtype), "sha256": numeric.array_digest(input_np)}, "private_materialization": {"scratch_scope": "private temporary directory; deleted after child", "input_bytes": {"bytes": input_private_path.stat().st_size, "sha256": sha256_file(input_private_path)}, "onnx_reference_bytes": {"bytes": reference_private_path.stat().st_size, "sha256": sha256_file(reference_private_path)}, "raw_bytes_published": False}, "tensorrt": {"shape": list(trt_output.shape), "dtype": str(trt_output.dtype), "finite": True, "sha256": numeric.array_digest(trt_output), "detections": trt_detections}, "onnx_cpu_reference": {"shape": list(ort_output.shape), "dtype": str(ort_output.dtype), "finite": True, "sha256": numeric.array_digest(ort_output), "detections": ort_detections}, "comparison": descriptive_primary_comparison(runtime["np"], ort_output, trt_output, model), "native_forward": False})
                state["records_written"] += 1
        telemetry_after = snapshot_fn(desktop, background, int(plan["runtime_requirements"]["device_argument"]))
...
def dispatch_children(repo: Path, plan: dict[str, Any], output_root: Path, models: list[str], timeout_seconds: int, run_fn: Callable[..., Any] = subprocess.run) -> tuple[list[dict[str, Any]], bool]:
    ...
    for model in models:
        ...
        command = [sys.executable, str(script), "--child", "--repo-root", str(repo), "--model", model, "--plan", str(output_root / "smoke_plan.json")]
        try:
            ...
            output = (exc.stdout or "") + (exc.stderr or "") if isinstance(exc.stdout, str) or isinstance(exc.stderr, str) else "child timed out"
            result = None
        else:
            output = (result.stdout or "") + (result.stderr or "")
        write_text_no_overwrite(output_root / "logs" / f"{model}.log", output)
        ...
        if report_path.is_file():
            ...
            row = child_failure(plan, model, _child_state(model), RuntimeError("child exited without report"), output_root)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", choices=("all",), default="all")
